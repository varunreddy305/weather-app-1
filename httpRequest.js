...
const httpRequestToGetCoordinates = (location, callback) => {
	...
	if (location) {
		app
			...
			.then(({ data }) => {
				...
				callback(undefined, { lattfromApi, place, placeLocation });
			})
			.catch(e => {
...
const httpRequestToGetWeather = (coordinates, place, callback) => {
	...
	app
		...
		.then(({ data }) => {
			...
			const message = `Currently the weather forecast in ${place} is ${data.current.weather_descriptions[0]}. The current temperature is ${data.current.temperature} and it feels like ${data.current.feelslike}. Wind speed is ${data.current.wind_speed}mph`;
			callback(undefined, { message, place });
		})
		.catch(e => {
			callback(fileReader(path.join(__dirname, 'weather.json')), undefined);
		});
};
